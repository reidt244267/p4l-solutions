...
def simulate_multiple_elections(
    ...
    """
    if num_trials<=0:
        raise ValueError("Invalid number of trials given")
    ...
    for _ in range(num_trials):
        #in each trial, simulate one election and figure out who won
        votes_1,votes_2=simulate_one_election(electoral_votes,polling_data,margin_of_error)
    ...
    #probablities
    probablity_1=win_count_1/num_trials
    probablity_2=win_count_2/num_trials
    probablity_tie=tie_count/num_trials

    return probablity_1, probablity_2, probablity_tie

import random # this should be helpful!

# Write your simulate_one_election() function here along with any subroutines that you need.
def simulate_one_election(electoral_votes: dict[str, int],polling_data: dict[str, float],
    margin_of_error: float
) -> tuple[int, int]:    
    """
